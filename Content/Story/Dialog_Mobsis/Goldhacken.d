...
FUNC VOID WeirdRock_S1 ()
{
	...
	{	
		self.aivar[AIV_INVINCIBLE]=TRUE;
		PLAYER_MOBSI_PRODUCTION	=	MOBSI_WEIRDROCK;
		Ai_ProcessInfos (her);
	};
}; 
var int FinishedDig;

INSTANCE PC_WeirdRock_Get (C_Info)
{
	npc				= PC_Hero;
	nr				= 2;
	condition		= PC_WeirdRock_Get_Condition;
	information		= PC_WeirdRock_Get_Info;
	permanent		= TRUE;
	description		= "Od³up trochê..."; 
};

FUNC INT PC_WeirdRock_Get_Condition ()
{
	if ((PLAYER_MOBSI_PRODUCTION	==	MOBSI_WEIRDROCK) && (FinishedDig != 1))
	{	
		return TRUE;
	};
};

FUNC VOID PC_WeirdRock_Get_Info()
{
	PrintScreen  ("W skale znalaz³eœ nienaruszon¹ kamienn¹ tablicê.", -1,-1,FONT_Screen,3);
	CreateInvItems	(self, ItWr_StonePlate_Cave, 1);
	B_ENDPRODUCTIONDIALOG ();
	FinishedDig = 1;
};

INSTANCE PC_WeirdRock_NothingHere (C_Info)
{
	npc				= PC_Hero;
	nr				= 2;
	condition		= PC_WeirdRock_NothingHere_Condition;
	information		= PC_WeirdRock_NothingHere_Info;
	permanent		= TRUE;
	important 		= TRUE;
};

FUNC INT PC_WeirdRock_NothingHere_Condition ()
{
	if ((PLAYER_MOBSI_PRODUCTION ==	MOBSI_WEIRDROCK) && (FinishedDig == 1))
	{	
		return TRUE;
...
FUNC VOID PC_WeirdRock_NothingHere_Info()
{
	PrintScreen  ("Nic tu nie ma.", -1,-1,FONT_Screen,3);
	B_ENDPRODUCTIONDIALOG ();
};
